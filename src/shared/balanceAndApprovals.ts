...
export async function balanceAndApprovals(
  ...
  if (tokenType == 'ERC721') {
    ...
    const { _hex: seaportAllowanceHex } = <
      { _hex: string; _isBigNumber: true }
    >await contract.allowance(owner, seaportAddress);

    const currentAllowanceToBN = BigNumber.from(seaportAllowanceHex);
    const newSpendToBN = BigNumber.from(
      utils.parseUnits(<string>amount, decimal)
    );
    const totalNewSpend = currentAllowanceToBN.add(newSpendToBN);

    const approvalTxn = await contract.approve(seaportAddress, totalNewSpend);
    await approvalTxn.wait();
  }
}

export async function HandleFulfillmentApprovals(
  owner: boolean,
  address: string,
  order: RetrieveOrderResponse
) {
  let ERC721Address = '';
  let tokenID = '';
  ...
  if (!owner) {
    ...
    spendAmountArray = order.parameters.consideration.map(f =>
      BigNumber.from(f.startAmount)
    );
  } else {
  ...
  if (!owner) {
    ...
    const spendAmount = utils.formatUnits(
      spendAmountArray.reduce((acc: BigNumber, curr: BigNumber) =>
        acc.add(curr)
      ),
      decimalOfToken
    );
    ...
    await balanceAndApprovals(address, 'ERC721', contract, tokenID);
  }
}
