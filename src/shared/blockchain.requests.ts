...
async function FetchOwnedNFTs (walletAddress: string) {
  ...
  try {
    ...
    await GetAllNFTsForAddressStandard(walletAddress).then(({ERC1155Map, ERC721Map}) => {
      FinalERC1155Map = ERC1155Map;
      FinalERC721Map = ERC721Map;
    })
  }
...
async function GetAllNFTsForAddressStandard(walletAddress: string) : Promise<{ERC1155Map: Map<string, TokenIdentifier>, ERC721Map: Map<string, TokenIdentifier>}> {
  const [ERC721Map, ERC1155Map] : Map<string, TokenIdentifier>[] = await Promise.all([
...
async function FetchERC721TokensStandard(walletAddress: string) : Promise<Map<string, TokenIdentifier>> {
  const ERC721Map: Map<string, TokenIdentifier> = new Map();
  const ERC721Contract = NewPolygonCollectionContract_MumbaiInstance;
  let { _hex: balance } = <{_hex: string | number, _isBigNumber: boolean}> (await ERC721Contract.balanceOf(walletAddress));
  ...
  await Promise.all(awaitingTokenIDs)
...
async function FetchERC1155TokensOptions(walletAddress: string) : Promise<Map<string, TokenIdentifier>> {
  const isInt = (n: any) => { return parseInt(n) === n };
  const ERC1155Map: Map<string, TokenIdentifier> = new Map();

  try {
    const MoralisResponse = await MoralisRequestAndResponse(<string> process.env.ERC1155_CONTRACT_ADDRESS_MUMBAI, walletAddress);
    ...
    const [transferredToRes, transferredFromRes] = await Promise.all([
      axios.post(`${mumbaiScanURL}?module=logs&action=getLogs&fromBlock=${fromBlock}&toBlock=${toBlock}&address=${contractAddress}&topic0=${topic0}&topic0_3_opr=and&topic3=${hexedAddress}`)
        .then(response => response.data.result),
      axios.post(`${mumbaiScanURL}?module=logs&action=getLogs&fromBlock=${fromBlock}&toBlock=${toBlock}&address=${contractAddress}&topic0=${topic0}&topic0_2_opr=and&topic2=${hexedAddress}`)
        .then(response => response.data.result)
    ]);

    // Check each token for value token existence in smart contract
    // Needed to know whether it's transferrable or not
    const tokensToCheckForValueTokenExistence = await DeconstructEventLog(transferredToRes, transferredFromRes, walletAddress);
...
async function GetAllNFTsForAddressMoralis(walletAddress: string) : Promise<{ERC1155Map: Map<string, TokenIdentifier>, ERC721Map: Map<string, TokenIdentifier>}> {
  const MoralisResponse = await MoralisRequestAndResponse([<string> process.env.ERC721_CONTRACT_ADDRESS_MUMBAI, <string> process.env.ERC1155_CONTRACT_ADDRESS_MUMBAI], walletAddress);
...
async function DeconstructEventLog(transferredToRes: EventLog[], transferredFromRes: EventLog[], walletAddress: string) : Promise<string[]> {
  const transferredToAndFromMap: Map<string, {tokenID: string, amount: number}> = new Map();
  ...
  transferredToAndFromMap.forEach((v, k) => {
    tokensToCheckForValueTokensPromise.push(CheckOwnerOfERC1155Token(k, walletAddress, ERC1155Contract))
  })

  const removeUnowned = await Promise.all(tokensToCheckForValueTokensPromise).then((res) => {
...
async function CheckOwnerOfERC1155Token(tokenID: string, walletAddress: string, ERC1155Contract: ethers.Contract) : Promise<null | string>{
  const {_hex} = <{ _hex: string, _isBigNumber: true }> await ERC1155Contract.balanceOf(walletAddress, tokenID);
...
async function MoralisRequestAndResponse(contractAddresses: string[] | string, walletAddress: string) : Promise<MoralisAddressNFTs[]> {
  const options: AxiosRequestConfig = {
    ...
    headers: {accept: 'application/json', 'X-API-Key': 'Pw7D7seAn4GwKQCQgX07kdI2kTvSgaBM7OAvvkx6DQF2GRjPjKEYp7O3uu15YVPG'}
  }
