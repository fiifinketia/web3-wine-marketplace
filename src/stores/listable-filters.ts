  ...
  actions: {
    ...
    setAllFilters(options: ListableToken[]) {
      options.map(f => {
        ...
        if (
          !this.investmentGradeOptions.find(a => a.value === f.investmentGrade)
        )
    ...
    filterListableTokens(tokensToFilter: ListableToken[]) {
      ...
      const producer: null | string[] =
        this.producer.length > 0 ? this.producer : null;
      const productionCountry: null | string[] =
        this.country.length > 0 ? this.country : null;
      const region: null | string[] =
        this.region.length > 0 ? this.region : null;
      const origin: null | string[] =
        this.origin.length > 0 ? this.origin : null;
      const appellation: null | string[] =
        this.appellation.length > 0 ? this.appellation : null;
      const wineCase = this.wineCase.length > 0 ? this.wineCase : null;
      const format: null | string[] =
        this.format.length > 0 ? this.format : null;
      const investmentGrade: null | string[] =
        this.investmentGrade.length > 0 ? this.investmentGrade : null;
      const lwin: null | string[] = this.LWIN.length > 0 ? this.LWIN : null;
      const heritage: null | string[] =
        this.heritage.length > 0 ? this.heritage : null;
    ...
    filterBasedOnKey(filters: any, tokensToFilter: ListableToken[]) {
      ...
      if (keys.length > 1) {
        keys.forEach(f => {
          ...
          this.filteredListableTokens = this.filteredListableTokens.filter(
            f => !!filters[key].includes(f[`${key}`])
          );
        });
        ...
        keys.forEach(f => {
          const key = f as keyof ListableToken;
          this.filteredListableTokens = tokensToFilter.filter(
            f => !!filters[key].includes(f[`${key}`])
          );
        });
      }
    ...
    filterBrand(brands: string[], originalListOfTokens: ListableToken[]) {
      ...
      if (this.filteredListableTokens.length !== originalListOfTokens.length) {
        this.filteredListableTokens = this.filteredListableTokens.filter(
          f => !!brands.includes(f['brand'])
        );
      } else {
        this.filteredListableTokens = originalListOfTokens.filter(
          f => !!brands.includes(f['brand'])
        );
      }
    },
